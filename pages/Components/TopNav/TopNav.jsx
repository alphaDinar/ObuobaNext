...
const TopNav = () => {
  ...
  useEffect(() => {
    getDoc(doc(fireStoreDB, "Obuoba/con")).then((res) => {
      ...
      const dailyPrograms = res
        .data()
        .programs.filter((el) => parseInt(el.day) === day)
        .sort(sortByHour);
      ...
      dailyPrograms.map((el) => {
        ...
        if (dailyPrograms[currentProgramIndex + 1]) {
          ...
          setNextProgram(
            res
              .data()
              .programs.filter((el) => parseInt(el.day) === nextDay)
              .sort(sortByHour)[0]
          );
        }
      });
    ...
    <section className="top_nav">
      <div className="player">
        ...
        <button onClick={togglePlaying}>
          {playing ? icon("power_settings_new") : icon("play_arrow")}
        </button>
      ...
      <section className="left_box">
        ...
        <div>
          <Link href={"/about"}>
            <a>
              {" "}
              {icon("not_listed_location")} <span>About us</span>
            </a>
          </Link>
          <Link href={"/contact"}>
            <a href="">
          ...
          <Link href={"/terms"}>
            <a href="">
              {" "}
              {icon("assignment")} <span>Terms and Policies</span>
            </a>
          </Link>
        </div>
      </section>
